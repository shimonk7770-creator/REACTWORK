...
└── src/
    ...
    │   ├── Tanya.jsx             # עמוד תניא (Sefaria Tanya Yomi לכל יום נבחר, מקומי רק בכשל רשת)
...
**החלטה:** טקסט חומש/תהילים/תניא נטען מ-Sefaria API בזמן אמת לכל תאריך שנבחר; `dailyContent.js` משמש רק גיבוי אם הבקשה ל-Sefaria נכשלת (למשל אין רשת).
